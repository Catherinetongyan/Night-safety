...
class UserProfile:
    ...
    All weights are 0.0–1.0; 0 = ignore, 1 = maximum influence.

    Hard constraints (boolean) set an edge weight to infinity —
    the solver will never route through an impassable edge.
    """
    WALKING_SPEED_KMH = 5
    WALKING_distance_min : float = ((WALKING_SPEED_KMH/60)*1000)  # average walking speed

    # ── Soft weights (additive penalties) ──────────────────────────────
    w_distance    : float = 0.5   # raw length in metres
    w_travel_time : float = 0.5   # osmnx travel_time in seconds
    w_gradient    : float = 0.0   # slope steepness (0–1 normalised)
    w_surface     : float = 0.0   # unpaved/rough surface penalty
    w_lighting    : float = 0.0   # unlit streets at night
    w_safety_risk  : float = 0.0   # fast roads (danger proxy)

    # ── Hard constraints (edge becomes impassable) ──────────────────────
    require_lit           : bool = False   # filter any unlit edge
    require_smooth_surface: bool = False   # filter cobble/unpaved
    wheelchair_accessible : bool = False   # requires footway/path, no steps

    # ── Network selector ────────────────────────────────────────────────
    network_type : str = "walk"   # "walk" or "drive"

    def profile_key(self) -> str:
        """Stable hash used as part of the cache key."""
...
# ── Normalisation look-up tables (built once) ────────────────────────────

# Surface quality: lower = rougher, higher penalty
_SURFACE_PENALTY = {
    "asphalt"   : 0.0,
    "paved"     : 0.0,
    "concrete"  : 0.05,
...
def _get_str(edge_data: dict, key: str, default: str = "") -> str:
    """Safely extract a scalar string from an OSM edge attribute."""
...
def cost_function(edge_data: dict, profile: UserProfile) -> float:
    ...
    if profile.require_lit and lit == "no":
        return float("inf")
    ...
    # gradient: stored as absolute grade (0–1) if present, else 0
    gradient    = min(abs(float(edge_data.get("grade", 0.0))), 1.0)

    # surface: look up penalty, default to 0 (smooth)
    surface_pen = _SURFACE_PENALTY.get(surface, 0.0)
    lit_pen     = 0.0 if lit == "yes" else 1.0

    safety_risk  = 0  #edge_data.get("speed_kph", 30.0)


    # ── 3. Weighted sum ──────────────────────────────────────────────────
    # We normalise base costs so 100m ≈ 60s ≈ 1.0 reference unit
    cost = (
        profile.w_distance    * (length      / profile.WALKING_distance_min)
      + profile.w_travel_time * (travel_time / 60.0)
      + profile.w_gradient    * gradient
      + profile.w_surface     * surface_pen
      + profile.w_lighting    * lit_pen
...
class QueryResult:
    ...
    def __str__(self):
        ...
        return (
            ...
            f"\n  Users_profile_key    : {self.Users_profile_key }"
            f"\n  Distances from: {src}"
        )
...
def _build_graph(network_type: str = "walk", cache_path: str = "./bristol_graph.graphml"):
    """
        Download or fetches the Bristol graph once.
        Stores raw metadata on each edge; no weights are baked in.
        """
    if os.path.exists(cache_path):
        ...
        ox.settings.useful_tags_way = ox.settings.useful_tags_way + _EXTRA_TAGS
        G = ox.graph_from_place("Bristol, England", network_type=network_type)
        G = ox.add_edge_speeds(G)
        G = ox.add_edge_travel_times(G)
        ox.save_graphml(G, cache_path)

        # Attempt to add elevation grade (requires an elevation API key;
        # silently skip if unavailable — grade will default to 0.0)
        try:
...
def _build_laplacian(G, profile: UserProfile):
    """
    Conductance-weighted Laplacian using the dynamic cost function.
    w_ij = 1 / cost_function(edge_data, profile)

    Edges with infinite cost are simply skipped (zero conductance).
    """
    nodes    = list(G.nodes())
    n        = len(nodes)
    ...
    for u, v, data in G.edges(data=True):
        ...
        if cost == float("inf"):
            cost = 1e6  # instead of continue impassable for this profile

        i = node_idx[u]
        j = node_idx[v]
        w = 1.0 / cost              # conductance (here is where it slows down the road)
...
def _heat_method(G, source_node: int, profile: UserProfile,
    ...
    """
    Geodesic distances from source_node under the given profile.
    """
    L, nodes, node_idx = _build_laplacian(G, profile)
    n   = len(nodes)
    src = node_idx[source_node]

    # mean cost instead of mean risk_weight
...
def visualise_interactive(
    ...
    profile = UserProfile()  # or pass it in properly
...
class RoutingService:
    ...
    Cache behaviour
    ---------------
     On-demand geodesic routing with per-request UserProfiles.

    One shared graph. No weights baked in. Thread-safe: the graph
    is read-only; each solve allocates its own local matrices.

    Cache key: (origin_node_id, profile.profile_key())
    This means two users from the same origin with different
    preferences each get their own distance field.

    Results are kept in memory; subsequent queries from the same origin are O(1)
    dictionary lookups.

    t_factor   : Heat timestep scale (1.0 is a good default)
    """

    def __init__(self, network_type: str = "walk", t_factor: float = 1.0):
        self.network_type = network_type
        self.t_factor     = t_factor
        self._G           = _build_graph(network_type)
        # {(origin_node, profile_key): distances_dict}
    ...
    def query(
        ...
        if profile is None:
            profile = UserProfile()          # sensible defaults
        ...
        return QueryResult(
            ...
            Users_profile_key = profile.profile_key(),
            cache_hit         = cache_hit,
        )
    ...
    def visualise_interactive(
            ...
            dest_lat: Optional[float] = None,
            dest_lon: Optional[float] = None,
            output_file: str = "Route_heatmap.html",
            profile: UserProfile = None,
    ):
        """
        Render an interactive Folium map with:
...
def _interactive_cli():
    ...
    PROFILES = {
        # Fast, flat pedestrian
        "standard": UserProfile(
            w_distance=0.3, w_travel_time=0.7,
        ),
        # Wheelchair user — hard-constraint on surfaces + steps
        "wheelchair": UserProfile(
            w_distance=0.5, w_travel_time=0.5,
            w_gradient=0.8,
            wheelchair_accessible=True,
            require_smooth_surface=True,
        ),
        # Night-safe walker — penalises unlit routes heavily
        "night": UserProfile(
            w_distance=0.3, w_travel_time=0.4,
            w_lighting=0.8, w_safety_risk=0.5,
            require_lit=True,
        ),
        # Hill-avoider
        "flat_route": UserProfile(
            w_distance=0.3, w_travel_time=0.3,
            w_gradient=0.9,
        ),
    }
    service = RoutingService(network_type="walk")
