...
Three routes are compared side by side:
  ...
  B — Distance + Crime + Safety  (A star on length + crime penalty − safety bonus)
  C — Heat Method (full)     (gradient descent on φ field: distance + crime + safety)

Usage
-----
    python bristol_heat_routing_gradient_fixed3.py
...
Fixes over bristol_heat_routing_gradient_fixed2.py
---------------------------------------------------
  - CCTV/light scoring now uses ox.nearest_edges to assign each infrastructure
    point to its nearest edge, instead of a 40m buffer circle that could bleed
    counts from adjacent/parallel streets
"""
...
GRAPH_PKL  = "bristol_heat_graph_v3.pkl"
...
def build_or_load_graph() -> nx.MultiDiGraph:
    ...
    # ── CCTV + lighting — assign each point to its nearest edge ──────────────
    # (No buffer — each CCTV/light is assigned to exactly one edge, avoiding
    #  bleed from adjacent/parallel streets)
    print("  Scoring edges for CCTV + lighting (nearest-edge assignment) …")
    cctv_df   = pd.read_csv(CCTV_CSV)
    lights_df = pd.read_csv(LIGHTS_CSV)

    print(f"    Assigning {len(cctv_df):,} CCTV to nearest edges …")
    cctv_ne  = ox.nearest_edges(G, cctv_df["longitude"].values, cctv_df["latitude"].values)
    print(f"    Assigning {len(lights_df):,} lights to nearest edges …")
    light_ne = ox.nearest_edges(G, lights_df["longitude"].values, lights_df["latitude"].values)

    # Count how many CCTV / lights are assigned to each (u, v, key)
    cctv_counts  = Counter(map(tuple, cctv_ne))
    light_counts = Counter(map(tuple, light_ne))

    # Mirror counts to reverse edges so both directions of a street benefit.
    # nearest_edges only returns one directed edge, but the old midpoint
    # approach scored both (u,v) and (v,u) identically — keep that behaviour.
    def mirror_counts(counts, G):
        mirrored = dict(counts)
        for (u, v, k), c in list(counts.items()):
            if G.has_edge(v, u):
                for rev_k in G[v][u]:
                    key = (v, u, rev_k)
                    mirrored[key] = max(mirrored.get(key, 0), c)
        return mirrored

    cctv_per_edge  = mirror_counts(cctv_counts, G)
    light_per_edge = mirror_counts(light_counts, G)

    raw_scores = {}
    for u, v, k, data in G.edges(keys=True, data=True):
        n_cctv  = cctv_per_edge.get((u, v, k), 0)
        n_light = light_per_edge.get((u, v, k), 0)
...
def recover_path(G, origin_node, dest_node, distances, profile: UserProfile) -> list:
    ...
    for _ in range(len(G.nodes)):
        ...
        if best_node is None:
            print(f"\n  ╔{'═'*60}╗")
            print(f"  ║  ⚠  FALLBACK: Gradient descent stuck at node {current}")
            print(f"  ║     (φ={phi_here:.4f}, visited {len(visited)} nodes)")
            print(f"  ║     Route C is using Dijkstra full-cost shortest path")
            print(f"  ╚{'═'*60}╝\n")
            break
    ...
    if current != origin_node:
        print(f"\n  ╔{'═'*60}╗")
        print(f"  ║  ⚠  FALLBACK: Gradient descent did not reach origin")
        print(f"  ║     (stopped {len(visited)} nodes in)")
        print(f"  ║     Route C is using Dijkstra full-cost shortest path")
        print(f"  ╚{'═'*60}╝\n")
        try:
            return nx.shortest_path(G, origin_node, dest_node,
                                    weight=lambda u, v, d: cost_function(d, profile))
...
def print_safety_analysis(stats_a, stats_b, stats_c):
    print("\n========== SAFETY ANALYSIS ==========")
    print(f"{'Metric':<25} {'A (shortest)':>14} {'B (+cri+saf)':>14} {'C (heat)':>14}")
    print("-" * 70)
...
def plot_routes(G, origin_node, dest_node,
    ...
    titles  = [
        ...
        f"Shortest Distance + Crime + Safety\n{dist_b:.0f}m  •  {format_time(mins_b)} walking",
        f"Geodesic distance estimation via heat diffusion\n{dist_c:.0f}m  •  {format_time(mins_c)} walking",
    ]
    ...
    print(f"  B  A* + Crime + Safety:  {dist_b:.0f}m  |  {format_time(mins_b)}")
...
def main():
    # ── Edit these ────────────────────────────────────────────────────────────
    ORIGIN_LAT, ORIGIN_LON = 51.45617, -2.60462  # Wills Memorial
    DEST_LAT,   DEST_LON   = 51.4491, -2.5814   # Temple Meads
    ORIGIN_NAME = "Wills Memorial"
    DEST_NAME   = "Temple Meads"
    ...
    t0      = time.time()
    route_c = recover_path(G, origin_node, dest_node, distances, profile)
    print(f"  Done: {time.time()-t0:.3f}s  |  {len(route_c)} nodes")

    if not route_c:
        print(f"\n  ╔{'═'*60}╗")
        print(f"  ║  ⚠  FALLBACK: No route produced by heat method or Dijkstra")
        print(f"  ║     Route C panel will show Route A (distance only)")
        print(f"  ╚{'═'*60}╝\n")
        route_c = route_a
